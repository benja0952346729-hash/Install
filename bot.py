...
from userbot import init_userbot_db, register_userbot_handlers
import random
...
async def _handle_group_message_inner(update, ctx, msg, user_id, user_name, text, group_id):
    ...
    if not parse_result.get("is_clear_pattern", True):
        try:
            ...
            if jina_is_ready():
                _jina_intent, _jina_score = await jina_detect_intent(text)
                if _jina_intent != "booking" or _jina_score < 0.5:
                    if resp["reply"]:
                        await msg.reply_text(resp["reply"])
                    return
        except Exception as _jina_err:
            import logging as _log
            _log.getLogger(__name__).warning(f"[JinaBookingCheck] Error: {_jina_err}")
    # ══════════════════════════════════════════════════════════════
