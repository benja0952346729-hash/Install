        ...
        try:
            ...
            async def handler(event, acc=account):
                try:
                    ...
                    if db_is_user_added(user_id, target_group_id):
                        logger.info(f"[DEBUG] User {user_id} already added!")
                        return

                    if (db_get_setting("auto_add_enabled") or "true") == "false":
                        logger.info(f"[DEBUG] Auto-add is disabled!")
                        return
                    ...
                    logger.info(f"[DEBUG] Adding {user_id} → {target_group_id} via [{chosen['label']}]")
